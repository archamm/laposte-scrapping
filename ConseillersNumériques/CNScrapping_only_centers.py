...
def scrap_results(centre, df, address_parser):
    ...
    row = [centre_id , center_coordinates, centre_nom, 
...
if __name__ == '__main__':
    ...
    df = pd.DataFrame(
        columns=['centre_id' , 'center_coordinates', 'centre_nom', 'centre_isLabeledAidantsConnect', 
        'centre_isLabeledFranceServices', 'centre_adress', 'centre_postal_code', 'centre_town', 'centre_openingHours'])
    f = open(PATH)
    data = json.load(f)
    ...
    df.to_csv('ConseillersNumériques/extract/extract_CN_only_centers.csv', encoding='utf-8-sig', header=True, sep=';')
